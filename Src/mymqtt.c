...
err_t recv_callback(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err) {
    ...
    else
    {
    	...
    	printf("mqttConnection closed by client.\r\n");
        tcp_close(pcb);
    }
    pbuf_free(p);
    return ERR_OK;
...
err_t accept_callback(void *arg, struct tcp_pcb *npcb, err_t err) {
    ...
    printf("mqttRecieve from broker.\n");
    //UARTprintf("\r\n");
...
void mqttInit(Mqtt *this, ip4_addr_t serverIp, int port, msgReceived fn, char *devId){
...
uint8_t mqttConnect(Mqtt *this) {
    ...
    if(err != ERR_OK){
    	printf("mqttConnection Error : %d\r\n",err);
    	mqttDisconnectForced(this);
        return 1;
    } else {
    	printf("mqttConnection sucessed..\r\n");
    }
